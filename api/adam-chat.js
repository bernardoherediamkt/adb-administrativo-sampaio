...
const { buildDriveContext, buildFastFinanceReport, buildFastCategoryExpenseReport, inferExpenseCategoryQuery, inferFinanceQuestion, requestedMonths, requestedYears } = require('./_drive-context');
...
- Você atende usuários autorizados da equipe da ADB Administrativo.
...
- Não presuma que quem está falando é o Pastor Bernardo.
- Não chame o usuário de “Pastor Bernardo”, “pastor” ou por qualquer nome, a menos que ele tenha se identificado nesta conversa.
- Quando precisar se dirigir à pessoa, use tratamento genérico como “posso te ajudar” ou “para gerar esse relatório”.
- Se o nome/cargo for importante, pergunte: “Como posso te chamar?”.
...
REGRA FINAL DE TRATAMENTO AO USUÁRIO:
A memória pode citar Pastor Bernardo como fundador, pastor ou referência de estilo, mas você NÃO deve presumir que o usuário atual é ele.
Trate a pessoa de forma genérica. Só chame por nome ou cargo se ela se identificar nesta conversa.
`, 120000);
}
...
Use o contexto do Drive/Planilhas SOMENTE da igreja selecionada para dados administrativos e financeiros. Para planilhas, use a GOOGLE SHEETS API como fonte oficial do conteúdo; Drive API serve apenas para localizar IDs/metadados quando necessário.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (shouldUseFastFinanceReport(message) && inferExpenseCategoryQuery(message)) {
      const fast = await buildFastCategoryExpenseReport(message, { churchId });
      return res.status(200).json({
        answer: fast.answer,
        model: 'sheets-api-category-expense-report',
        driveConnected: true,
