...
// Hospede em Vercel/servidor Node. Não coloque sua chave no index.html.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!geminiResponse.ok) {
      const message = json?.error?.message || 'Erro na API do Gemini.';
      return res.status(geminiResponse.status).json({ detail: message, raw: json?.error || null });
    }

    const outputParts = json?.candidates?.[0]?.content?.parts || [];
    const imagePart = outputParts.find(part => part.inlineData && part.inlineData.data);
    const textPart = outputParts.find(part => part.text)?.text || '';
    ...
    return res.status(500).json({
      detail: error?.message || 'Erro interno ao gerar a arte.'
    });
  }
}
